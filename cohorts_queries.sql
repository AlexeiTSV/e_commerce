...
-- Набор данных содержит информацию о транзакциях покупателей розничного онлайн-магазина.
-- Для когортного анализа в основном используются поля customerid (id покупателя) и invoicedate (дата заказа). 
...
-- 4.2. такой же запрос что и в п.4.1., но подготовленный для вывода на график дашборда
select 
	months
